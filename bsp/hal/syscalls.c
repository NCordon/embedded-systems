...
int _open(const char *pathname, int flags, mode_t mode){
  ...
  // Se devuelve -1 si la función open falla o no hay función open, o si el dispositivo no existe
  return -1;
}
...
int _close (int fd){
  ...
  // Se devuelve -1 si la función close falla o no hay función close, o si el dispositivo no existe
  return -1;
}
...
ssize_t _read(int fd, char *buf, size_t count){
  ...
  // Devuelve el número de bytes leídos si la función read no falla
  //  Si falla devuelve -1
...
ssize_t _write (int fd, char *buf, size_t count){
  ...
  // Devuelve el número de bytes escritos si la función read no falla
  //  Si falla devuelve -1
  // Si no existe devuelve el número de bytes a escribir
  if(dev && dev -> write)
 ...
 * @return              El nuevo desplazamiento medido en bytes 
 *                      desde el inicio del fichero o -1 en caso de error.
 * 			La condición de error se indica en la variable global errno
 */
off_t _lseek(int fd, off_t offset, int whence){
  bsp_dev_t *dev = get_dev(fd);

  // Devuelve 0 si no hay función lseek o no existe
  // -1 si falla
  // El buyes desplazados si todo ha funcionado correctamente
  if(dev && dev -> lseek)
...
int _isatty (int fd){
  ...
  if(dev && dev -> isatty)
    ...
    // Es un dispositivo de caracteres o no tiene función isatty, o  dev no existe
    return 1;
  }
}
